...
from src.constants import REPORT_TYPE
...
def run(args):
    if len(args) < 2:
        raise Exception("""Missing arguments.
            ...
            $ python run_freebies.py welcome-offer-reminder 2018-12-31
            """
        )
    ...
    if command_option == COMMAND_OPTIONS.all:
        print("Freebie Reporting system is running with %s option in %s mode..." % (COMMAND_OPTIONS.all, ENV))
        count = dog.report_for_all_data(report_type=REPORT_TYPE.freebies_report)
        ...
        count = dog.report_for_all_data(date_string, report_type=REPORT_TYPE.freebies_report)
    else:
        event_id = args[1]
        if len(args) > 3:
            subtype = args[2]
        ...
        count = dog.get_report(event_id, subtype, date_string, report_type=REPORT_TYPE.freebies_report)
